...
exports.GET_FLOATING_PINS = (req,res,next)=>{
    ...
    OTP.find({"mobile": { $exists: false, $ne: ""}}, {_id:0, pin:1,ts:1})
...
const biometricgetalltokens_pushNotification = function(){
    return new Promise((resolve,reject)=>{
        ...
        XCorona.find( 
            {expiry:"false",  biolastupdate: {$lt: ex}}, 
            { biolastupdate:1,expiry:1, _id: 0, pin:1, token:1}
        ).then((result)=>{
...
exports.GET_ALL_PINS = (req,res,next)=>{
    XCorona.find( {}, { expiry:1, _id: 1, pin:1, token:1, loastart:1, loaend:1,biolastupdate:1 } ).then((result)=>{
        console.log("jain", result)
        return res.status(200).json({
            message: result
        })
...
const getalltokens_pushNotification = function(){
    return new Promise((resolve,reject)=>{
		...
		{  expiry:"false", lastupdate: {$lt: ex}}, 
            { lastupdate:1,expiry:1, _id: 0, pin:1, token:1}
        ).then((result)=>{
            console.log(result)
            resolve(result)
            //   console.log("jain", result)
...
const checkExpiry = function(result){
    return new Promise((resolve,reject)=>{
        ...
        if (now >= expiryepoch) {
            //YES, SMALLER
            console.log("expired", now, expiryepoch)
            reject({
                status: 3,
                desc: "expired"
            })
            ...
            //NOPE
            console.log("not expired",now, expiryepoch )
            resolve(true)
        }
    })
}
